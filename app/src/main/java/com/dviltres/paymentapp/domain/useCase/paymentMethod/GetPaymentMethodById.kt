...
class GetPaymentMethodById @Inject constructor(
    private val repository: PaymentMethodRepository,
    private val application: Application,
) {
    operator fun invoke(paymentMethodId:String): Flow<Resource<PaymentMethod>> = flow {
        try {
            emit(Resource.Loading())
            val paymentMethod = repository.getPaymentMethodsById(paymentMethodId = paymentMethodId)
